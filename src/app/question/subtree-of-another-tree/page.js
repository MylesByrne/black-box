...
export default function ProblemPage() {
  const problemId = "subtree-of-another-tree"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      root_array = testCase.get("root", [])
      subRoot_array = testCase.get("subRoot", [])
      
      # Build the main tree (root)
      if not root_array or root_array == "None":
          root = None
      else:
          # Create nodes first
          nodes_root = [None if val is None or val == "None" else TreeNode(val) for val in root_array]
          # Connect nodes to form tree
          for j in range(len(nodes_root)):
              if nodes_root[j] is not None:
                  left_idx = 2 * j + 1
                  right_idx = 2 * j + 2
                  # Assign left child
                  if left_idx < len(nodes_root):
                      nodes_root[j].left = nodes_root[left_idx]
                  # Assign right child
                  if right_idx < len(nodes_root):
                      nodes_root[j].right = nodes_root[right_idx]
          
          root = nodes_root[0] if nodes_root else None
      
      # Build the subtree (subRoot)
      if not subRoot_array or subRoot_array == "None":
          subRoot = None
      else:
          # Create nodes first
          nodes_sub = [None if val is None or val == "None" else TreeNode(val) for val in subRoot_array]
          # Connect nodes to form tree
          for j in range(len(nodes_sub)):
              if nodes_sub[j] is not None:
                  left_idx = 2 * j + 1
                  right_idx = 2 * j + 2
                  # Assign left child
                  if left_idx < len(nodes_sub):
                      nodes_sub[j].left = nodes_sub[left_idx]
                  # Assign right child
                  if right_idx < len(nodes_sub):
                      nodes_sub[j].right = nodes_sub[right_idx]
          
          subRoot = nodes_sub[0] if nodes_sub else None
      
      # Call solution with both trees
      result = solution.${functionName}(root, subRoot)
      
      if str(result).lower() == str(testCase['expected']).lower():
          continue
      else:
          print(f"Test case {i+1}: Failed | Input: root={root_array}, subRoot={subRoot_array} | Expected: {testCase['expected']}, Got: {result}")
          return
