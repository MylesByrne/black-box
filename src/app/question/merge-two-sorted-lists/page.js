...
export default function ProblemPage() {
  const problemId = "merge-two-sorted-lists"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      list1_array = testCase.get("list1", [])
      list2_array = testCase.get("list2", [])
      
      # Create first linked list
      if not list1_array:
          list1 = None
      else:
          list1 = ListNode(list1_array[0])
          current = list1
          for val in list1_array[1:]:
      ...
      if not list2_array:
          ...
          list2 = ListNode(list2_array[0])
          current = list2
          for val in list2_array[1:]:
              current.next = ListNode(val)
              current = current.next
      
      # Call solution and convert result to array
      result = []
      current = solution.${functionName}(list1, list2)
      while current:
      ...
      if result == testCase['expected']:
          ...
          print(f"Test case {i+1}: Failed | Input: list1={testCase.get('list1', [])}, list2={testCase.get('list2', [])} | Expected: {testCase['expected']}, Got: {result}")
          return
