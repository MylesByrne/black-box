...
export default function ProblemPage() {
  const problemId = "same-tree"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      p_array = testCase.get("p", [])
      q_array = testCase.get("q", [])
      
      # Build the first binary tree (p)
      if not p_array or p_array == "None":
          p = None
      else:
          # Create nodes first
          nodes_p = [None if val is None or val == "None" else TreeNode(val) for val in p_array]
          # Connect nodes to form tree
          for j in range(len(nodes_p)):
              if nodes_p[j] is not None:
                  left_idx = 2 * j + 1
                  right_idx = 2 * j + 2
                  # Assign left child
                  if left_idx < len(nodes_p):
                      nodes_p[j].left = nodes_p[left_idx]
                  # Assign right child
                  if right_idx < len(nodes_p):
                      nodes_p[j].right = nodes_p[right_idx]
          
          p = nodes_p[0] if nodes_p else None
      
      # Build the second binary tree (q)
      if not q_array or q_array == "None":
          q = None
      else:
          # Create nodes first
          nodes_q = [None if val is None or val == "None" else TreeNode(val) for val in q_array]
          # Connect nodes to form tree
          for j in range(len(nodes_q)):
              if nodes_q[j] is not None:
                  left_idx = 2 * j + 1
                  right_idx = 2 * j + 2
                  # Assign left child
                  if left_idx < len(nodes_q):
                      nodes_q[j].left = nodes_q[left_idx]
                  # Assign right child
                  if right_idx < len(nodes_q):
                      nodes_q[j].right = nodes_q[right_idx]
          
          q = nodes_q[0] if nodes_q else None
      
      # Call solution with both trees
      result = solution.${functionName}(p, q)
      
      if str(result).lower() == str(testCase['expected']).lower():
          continue
      else:
          print(f"Test case {i+1}: Failed | Input: p={p_array}, q={q_array} | Expected: {testCase['expected']}, Got: {result}")
          return
