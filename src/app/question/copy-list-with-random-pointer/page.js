...
export default function ProblemPage() {
  const problemId = "copy-list-with-random-pointer"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      if not head_list:
          ...
          nodes = []
          for val_random in head_list:
              val = val_random[0]
              nodes.append(Node(val))
          
          # Second pass: connect next and random pointers
          for j, val_random in enumerate(head_list):
              if j < len(nodes) - 1:
                  nodes[j].next = nodes[j + 1]
              
              random_idx = val_random[1]
              if random_idx != -1 and random_idx != "None":
                  nodes[j].random = nodes[random_idx]
          
          head = nodes[0]
      
      # Call the solution
      copied_head = solution.${functionName}(head)
      
      # Basic verification (values only, not structure)
      result = []
      current = copied_head
      while current:
          # Store both value and random pointer's value if exists
      ...
      while current:
          random_val = current.random.val if current.random else None
          expected.append([current.val, random_val])
          current = current.next

      if result == expected:
          continue
      else:
          print(f"Test case {i+1}: Failed | Input: head={testCase['head']} | Expected structure not matched")
          return
          
      # Additional check to ensure it's a deep copy
      original = head
      copied = copied_head
      while original and copied:
          if original is copied:  # Check if they're the same object
              print(f"Test case {i+1}: Failed | Not a deep copy")
              return
