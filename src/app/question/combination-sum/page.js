...
export default function ProblemPage() {
  const problemId = "combination-sum"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      candidates = testCase.get("candidates", [])
      target = testCase.get("target", 0)
      expected = testCase.get("expected", [])
      
      # Call the combinationSum function with candidates and target
      result = solution.combinationSum(candidates, target)
      
      # Sort both result and expected for consistent comparison
      # First sort each combination, then sort the list of combinations
      sorted_result = sorted([sorted(combo) for combo in result])
      sorted_expected = sorted([sorted(combo) for combo in expected])
