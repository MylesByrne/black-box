...
export default function ProblemPage() {
  const problemId = "detect-squares"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      operations = testCase.get("operations", [])
      params = testCase.get("arguments", [])
      expected = testCase.get("expected", [])
      
      # Create a new instance of DetectSquares for this test case
      obj = None
      results = []
      ...
      for j in range(len(operations)):
          ...
          if op == "DetectSquares":
              ...
              # Keep as list since student's implementation expects List[int]
              if isinstance(param, list) and len(param) == 2:
                  x, y = int(param[0]), int(param[1])
                  obj.add([x, y])  # Pass as list, not tuple
              else:
                  obj.add(param)
              results.append(None)
          elif op == "count":
              ...
              if isinstance(param, list) and len(param) == 2:
                  x, y = int(param[0]), int(param[1])
                  result = obj.count([x, y])  # Pass as list, not tuple
              else:
                  result = obj.count(param)
              results.append(result)
          
      # Compare results with expected output
      # Only compare non-None results (from count operations)
      comparison_results = [r for r in results if r is not None]
      comparison_expected = [e for e in expected if e is not None]
      
      if comparison_results == comparison_expected:
          continue
      else:
          print(f"Test case {i+1}: Failed")
          print(f"Operations: {operations}")
          print(f"Parameters: {params}")
          print(f"Expected: {comparison_expected}")
          print(f"Got: {comparison_results}")
          return
