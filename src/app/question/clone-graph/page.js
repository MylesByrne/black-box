...
export default function ProblemPage() {
  const problemId = "clone-graph"; // Hardcoded
...
# Definition for a Node in the graph problem
class Node:
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      if not adjList:
          ...
          # Create nodes first
          nodes = {}
          for j in range(1, len(adjList) + 1):  # 1-indexed nodes in this problem
              nodes[j] = Node(j)
          
          # Connect nodes based on adjacency list
          for j in range(len(adjList)):
              node_idx = j + 1  # 1-indexed
              for neighbor_idx in adjList[j]:
                  nodes[node_idx].neighbors.append(nodes[neighbor_idx])
          
          # Return the first node as the entry point
          node = nodes[1]
      
      # Call the cloneGraph function
      cloned = solution.cloneGraph(node)
      
      # Verify the cloned graph structure by checking node values and connections
      if node is None:
          if cloned is None:
              continue  # Both are None, this is correct
          else:
              print(f"Test case {i+1}: Failed")
              print(f"Input was None but got a non-None result")
              return
      
      # Convert both original and cloned graphs to sets of edges for order-independent comparison
      original_edges = set()
      cloned_edges = set()
      
      # BFS to traverse original graph and collect edges
      if node:
          visited = set()
          queue = deque([node])
          visited.add(node)
          ...
          while queue:
              ...
              for neighbor in curr.neighbors:
                  # Add edge (smaller val, larger val) to ensure consistent ordering
                  edge = (min(curr.val, neighbor.val), max(curr.val, neighbor.val))
      ...
      # BFS to traverse cloned graph and collect edges
      if cloned:
          visited = set()
          queue = deque([cloned])
          visited.add(cloned)
          ...
          while queue:
              ...
              for neighbor in curr.neighbors:
                  # Add edge (smaller val, larger val) to ensure consistent ordering
                  edge = (min(curr.val, neighbor.val), max(curr.val, neighbor.val))
                  cloned_edges.add(edge)
                  
                  if neighbor not in visited:
                      visited.add(neighbor)
                      queue.append(neighbor)
      
      # Check if all nodes and edges match
      if original_edges == cloned_edges:
          continue
      else:
          print(f"Test case {i+1}: Failed")
          print(f"Input adjList: {adjList}")
          print(f"Original graph edges: {sorted(original_edges)}")
          print(f"Cloned graph edges: {sorted(cloned_edges)}")
          return
