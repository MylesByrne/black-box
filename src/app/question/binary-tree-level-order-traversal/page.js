...
export default function ProblemPage() {
  const problemId = "binary-tree-level-order-traversal"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      # Call solution
      result = solution.${functionName}(root)
      
      # Compare results - special handling for level order traversal
      # which returns a list of lists
      if result == testCase.get('expected', []):
          continue
      else:
          print(f"Test case {i+1}: Failed | Input: root={root_array} | Expected: {testCase.get('expected', [])}, Got: {result}")
          return
