...
export default function ProblemPage() {
  const problemId = "serialize-and-deserialize-binary-tree"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      # Build the binary tree from array
      if not root_array or root_array == "None":
          root = None
      else:
          # Create nodes first
          nodes = [None if val is None or val == "None" else TreeNode(val) for val in root_array]
      ...
      deserialized_root = solution.deserialize(serialized)
      
      # Convert the deserialized tree to level-order array for comparison
      result = []
      if deserialized_root:
          # Level-order traversal
          queue = [deserialized_root]
          while queue:
              node = queue.pop(0)
              if node:
          ...
          while result and (result[-1] is None or result[-1] == "None"):
              result.pop()
      
      # Convert None values in expected to string "None" for comparison
      ...
      if result == expected:
          ...
          print(f"Test case {i+1}: Failed | Input: root={root_array} | Expected: {expected}, Got: {result}")
          return
