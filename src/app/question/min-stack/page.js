...
export default function ProblemPage() {
  const problemId = "min-stack"; // Hardcoded
...
def check_solution():
    ...
    for i in range(len(testCases)):
        ...
        for j, (command, input_args) in enumerate(zip(commands, values)):
            try:
                if command == "MinStack":
                    obj = MinStack()
                    results.append("None")
                    ...
                    if input_args:
                        result = getattr(obj, command)(*input_args)
                    else:
                        result = getattr(obj, command)()
                    if result is None:
                        result = "None"
                ...
                print(f"Test case {i+1}: Error in command {command}: {e}")
                return
        
        # Compare results with expected
        if results != expected:
            print(f"Test case {i+1}: Failed | Commands: {commands} | Values: {values} | Expected: {expected}, Got: {results}")
            return
...
try:
    ...
    print("Error:", e)
        ...
        // Handle regular function-based problems
        testCall = `
from typing import List
from collections import defaultdict
...
def compare_nested_arrays(arr1, arr2):
    """Compare two nested arrays ignoring order at all levels"""
    ...
    for item1 in arr1:
        ...
        for i, item2 in enumerate(arr2_copy):
            if compare_nested_arrays(item1, item2):
                arr2_copy.pop(i);
                found_match = True;
                break;
        if not found_match:
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      if compare_nested_arrays(result, testCase['expected']):
        continue
      else:
