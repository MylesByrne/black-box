...
export default function ProblemPage() {
  const problemId = "find-the-duplicate-number"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      # Call the solution function with the array
      result = solution.${functionName}(nums)
      
      # For this problem, the result is a single number
      if result == testCase['expected']:
          continue
      else:
          print(f"Test case {i+1}: Failed | Input: nums={nums} | Expected: {testCase['expected']}, Got: {result}")
          return
