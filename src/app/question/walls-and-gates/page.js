...
export default function ProblemPage() {
  const problemId = "walls-and-gates"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      rooms = testCase.get("rooms", [])
      expected = testCase.get("expected", [])
      
      # Create a deep copy of the rooms matrix for the solution to modify
      input_rooms = [row[:] for row in rooms]
      
      # Call the wallsAndGates function which modifies the rooms matrix in place
      solution.wallsAndGates(input_rooms)
      
      # Compare the modified rooms with expected output
      if input_rooms == expected:
          continue
      else:
