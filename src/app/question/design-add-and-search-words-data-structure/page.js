...
export default function ProblemPage() {
  const problemId = "design-add-and-search-words-data-structure"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      inputs = testCase.get("values", [])
      expected = testCase.get("expected", [])
      
      # Create a new WordDictionary instance for each test case
      ...
      for j in range(len(operations)):
          ...
          if operation == "WordDictionary":
              # Constructor - already created the WordDictionary instance
              actual_results.append(None)
          elif operation == "addWord":
              result = word_dict.addWord(args[0])
              # addWord typically returns None
              actual_results.append(result)
          elif operation == "search":
              result = word_dict.search(args[0])
              actual_results.append(result)
          else:
              actual_results.append(None)
              print(f"Unknown operation: {operation}")
      
      # Convert actual_results and expected for proper comparison
      # This handles "None" strings vs None values and boolean values
      normalized_results = []
      for result in actual_results:
          if result is None:
              normalized_results.append("None")
          elif result is True:
              normalized_results.append(True)
          elif result is False:
              normalized_results.append(False)
          else:
      ...
      for exp in expected:
          if exp is None:
              ...
              normalized_expected.append(True)
          elif exp == "False":
              normalized_expected.append(False)
          else:
      ...
      if normalized_results == normalized_expected:
          ...
          if mismatch_index >= 0:
              ...
              print(f"Input: {inputs[mismatch_index]}")
