...
export default function ProblemPage() {
  const problemId = "reverse-nodes-in-k-group"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      head_array = testCase.get("head", [])
      k = testCase.get("k", 1)  # Get the k value for grouping
      
      # Handle empty linked list case
      if not head_array or head_array == "None":
          head = None
      else:
          # Create linked list
      ...
      result = []
      current = solution.${functionName}(head, k)  # Pass both head and k
      while current:
      ...
      if result == testCase['expected']:
          ...
          print(f"Test case {i+1}: Failed | Input: head={head_array}, k={k} | Expected: {testCase['expected']}, Got: {result}")
          return
