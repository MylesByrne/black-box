...
export default function ProblemPage() {
  const problemId = "add-two-numbers"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      l1_array = testCase.get("l1", [])
      l2_array = testCase.get("l2", [])
      
      # Create first linked list
      if not l1_array or l1_array == "None":
          l1 = None
      else:
          l1 = ListNode(l1_array[0])
          current = l1
          for val in l1_array[1:]:
              current.next = ListNode(val)
              current = current.next
      
      # Create second linked list
      if not l2_array or l2_array == "None":
          l2 = None
      else:
          l2 = ListNode(l2_array[0])
          current = l2
          for val in l2_array[1:]:
              current.next = ListNode(val)
              current = current.next
      
      # Call the solution with both lists
      result = []
      current = solution.${functionName}(l1, l2)
      while current:
      ...
      if result == testCase['expected']:
          ...
          print(f"Test case {i+1}: Failed | Input: l1={testCase.get('l1', [])}, l2={testCase.get('l2', [])} | Expected: {testCase['expected']}, Got: {result}")
          return
