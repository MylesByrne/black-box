...
export default function ProblemPage() {
  const problemId = "longest-palindromic-substring"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      # Call the longestPalindrome function with the input string
      result = solution.longestPalindrome(s)
      
      # Check if the result is one of the valid palindromic substrings
      # (There might be multiple palindromes of the same length)
      if len(result) == len(expected) and is_palindrome(result) and result in s:
          # If lengths match and result is a valid palindrome contained in the input
          if result == expected or is_equivalent_palindrome(s, result, expected):
              continue
      ...
      if result == expected:
          ...
          # Additional validation to help debugging
          if result not in s:
              print("Error: Result is not a substring of the input")
          elif not is_palindrome(result):
              print("Error: Result is not a palindrome")
          elif len(result) != len(expected):
              print(f"Error: Length mismatch - expected length {len(expected)}, got length {len(result)}")
          return
...
# Helper function to check if two palindromes are equivalent solutions
# (both are valid longest palindromic substrings)
def is_equivalent_palindrome(original, result, expected):
    # Both must be palindromes
    if not is_palindrome(result) or not is_palindrome(expected):
        return False
    
    # Both must be the same length (to be the longest)
    if len(result) != len(expected):
        return False
    
    # Both must be substrings of the original string
    if result not in original or expected not in original:
        return False
