...
export default function ProblemPage() {
  const problemId = "surrounded-regions"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      board = testCase.get("board", [])
      expected = testCase.get("expected", [])
      
      # Create a deep copy of the board for the solution to modify
      input_board = [row[:] for row in board]
      
      # Call the solve function which modifies the board in place
      solution.solve(input_board)
      
      # Compare the modified board with expected output
      if input_board == expected:
          continue
      else:
