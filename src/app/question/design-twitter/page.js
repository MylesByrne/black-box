...
export default function ProblemPage() {
  const problemId = "design-twitter"; // Hardcoded
  ...
  if (error || !problem) {
    ...
    try {
      ...
      // Check if this is a class-based problem like MinStack
      const isClassBased = problemId === 'min-stack' || problem.isClassBased;
      
      let testCall;
      
        // Handle class-based problems like MinStack
        testCall = `from typing import List, Optional
import heapq
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      operations = testCase.get("commands", [])
      arguments = testCase.get("values", [])
      expected = testCase.get("expected", [])
      
      # Create a new Twitter instance for each test case
      twitter = Twitter()
      results = []
      ...
      for j in range(len(operations)):
          ...
          # Execute the operation based on the method name
          if operation == "Twitter":
              # Constructor - already created the Twitter instance
              results.append(None)
          elif operation == "postTweet":
              result = twitter.postTweet(args[0], args[1])
              results.append(result)
          elif operation == "getNewsFeed":
              result = twitter.getNewsFeed(args[0])
              results.append(result)
          elif operation == "follow":
              result = twitter.follow(args[0], args[1])
              results.append(result)
          elif operation == "unfollow":
              result = twitter.unfollow(args[0], args[1])
              results.append(result)
          else:
              results.append(None)
              print(f"Unknown operation: {operation}")
      
      # Convert results and expected for proper comparison
      # Convert None to "None" for string comparison
      normalized_results = []
      ...
      for exp in expected:
          if exp is None:
              ...
              normalized_expected.append(exp)
      
      # Compare normalized results with normalized expected output
      if normalized_results == normalized_expected:
          continue
      else:
          # Find the first mismatch for better error reporting
          mismatch_index = -1
          for j in range(min(len(normalized_results), len(normalized_expected))):
              if normalized_results[j] != normalized_expected[j]:
                  mismatch_index = j
                  break
          
          if mismatch_index >= 0:
              print(f"Test case {i+1}: Failed at operation {operations[mismatch_index]}")
              print(f"Input: {arguments[mismatch_index]}")
              print(f"Expected: {normalized_expected[mismatch_index]}, Got: {normalized_results[mismatch_index]}")
          else:
              print(f"Test case {i+1}: Failed | Expected: {normalized_expected}, Got: {normalized_results}")
          return
