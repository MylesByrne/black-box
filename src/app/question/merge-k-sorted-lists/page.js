...
export default function ProblemPage() {
  const problemId = "merge-k-sorted-lists"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      lists_array = testCase.get("lists", [])
      
      # Create array of linked lists
      lists = []
      for list_vals in lists_array:
          if not list_vals or list_vals == "None":
              lists.append(None)
              continue
              
          # Create a linked list for each subarray
      ...
      # Call solution with array of linked lists
      result = []
      current = solution.${functionName}(lists)
      while current:
      ...
      if result == testCase['expected']:
          ...
          print(f"Test case {i+1}: Failed | Input: lists={lists_array} | Expected: {testCase['expected']}, Got: {result}")
          return
