...
export default function ProblemPage() {
  const problemId = "kth-largest-element-in-a-stream"; // Hardcoded
  ...
  for i in range(len(testCases)):
      ...
      for j in range(len(operations)):
          ...
          if op == "KthLargest":
              ...
              results.append(None)  # Constructor doesn't return anything
          elif op == "add":
              val = args[0]
      ...
      # Compare results with expected output, ignoring the first None from constructor
      normalized_results = results[1:] if len(results) > 0 else []
      normalized_expected = expected[1:] if len(expected) > 0 else []
      ...
      if normalized_results == normalized_expected:
          ...
          if mismatch_index >= 0:
              add_op_index = mismatch_index + 1  # +1 because we skip the constructor
              print(f"Test case {i+1}: Failed at operation {operations[add_op_index]}")
              print(f"Input: {arguments[add_op_index]}")
