...
export default function ProblemPage() {
  const problemId = "lru-cache"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      # Convert string "None" to actual None in expected results
      ...
      for j, command in enumerate(commands):
          try:
              if command == "LRUCache":
                  ...
                  key = values[j][0]
                  results.append(lru_cache.get(key))
              else:
                  results.append(None)
      ...
      # Compare with handling for string "None" values
      for j in range(len(results)):
          if (results[j] != expected[j]) and not (results[j] is None and expected[j] == "None"):
