...
export default function ProblemPage() {
  const problemId = "lowest-common-ancestor-of-a-binary-search-tree"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      p_val = testCase.get("p")
      q_val = testCase.get("q")
      ...
      # Call solution with root, p, and q
      result = solution.${functionName}(root, p_node, q_node)
      
      # Check if result matches expected
      if result and result.val == testCase['expected']:
          continue
      else:
          result_val = result.val if result else None
          print(f"Test case {i+1}: Failed | Input: root={root_array}, p={p_val}, q={q_val} | Expected: {testCase['expected']}, Got: {result_val}")
          return
