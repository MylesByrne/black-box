...
export default function ProblemPage() {
  const problemId = "linked-list-cycle"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      # Call solution and get result (should be a boolean)
      result = solution.${functionName}(head)
      
      if str(result) == testCase['expected']:
          continue
      else:
          print(f"Test case {i+1}: Failed | Input: head={testCase['head']}, pos={pos} | Expected: {testCase['expected']}, Got: {result}")
          return
