...
export default function ProblemPage() {
  const problemId = "course-schedule-ii"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      numCourses = testCase.get("numCourses", 0)
      prerequisites = testCase.get("prerequisites", [])
      expected = testCase.get("expected", [])
      
      # Call the findOrder function with the number of courses and prerequisites
      ...
      if len(result) != len(expected):
          print(f"Test case {i+1}: Failed - Result length mismatch")
          print(f"Input numCourses: {numCourses}, prerequisites: {prerequisites}")
          print(f"Expected: {expected}, Got: {result}")
          continue
      
      # Check if result is one of the valid topological sorts
      # Build graph for validation
      graph = defaultdict(list)
      for dst, src in prerequisites:
          graph[src].append(dst)
      
      # Verify the ordering - for each course, all its prerequisites 
      # should appear earlier in the order
      courses_index = {}
      for idx, course in enumerate(result):
          courses_index[course] = idx
      
      valid_order = True
      for course in range(numCourses):
          for prereq in graph.get(course, []):
              if courses_index.get(course, -1) >= courses_index.get(prereq, -1):
                  valid_order = False
                  break
          if not valid_order:
              break
      
      if valid_order and set(result) == set(range(numCourses)):
          continue
      else:
          # Check if the result matches the expected output exactly
          if result == expected:
              continue
