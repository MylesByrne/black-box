...
export default function ProblemPage() {
  const problemId = "kth-smallest-element-in-a-bst"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      k = testCase.get("k", 1)  # Get the k parameter
      ...
      # Call solution with root and k
      result = solution.${functionName}(root, k)
      
      # For kthSmallest, the result is an integer
      if result == testCase['expected']:
          continue
      else:
          print(f"Test case {i+1}: Failed | Input: root={root_array}, k={k} | Expected: {testCase['expected']}, Got: {result}")
          return
