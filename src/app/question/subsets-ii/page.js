...
export default function ProblemPage() {
  const problemId = "subsets-ii"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      # Call the subsetsWithDup function with the input array
      result = solution.${functionName}(nums)

      # Sort both result and expected for consistent comparison
      # First sort each subset, then sort the list of subsets
      sorted_result = sorted([sorted(subset) for subset in result])
      sorted_expected = sorted([sorted(subset) for subset in expected])
