...
export default function ProblemPage() {
  const problemId = "time-based-key-value-store"; // Hardcoded
  ...
  if (error || !problem) {
    ...
    try {
      ...
      let testCall;
        ...
        testCall = `from typing import List
        ...
        for j, (command, input_args) in enumerate(zip(commands, values)):
            try:
                if command == "TimeMap":
                    ...
                    key, value, timestamp = input_args
                    obj.set(key, value, timestamp)
                    results.append("None")
                elif command == "get":
                    key, timestamp = input_args
                    results.append(obj.get(key, timestamp))
                else:
                    results.append("None")
