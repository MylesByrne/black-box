...
export default function ProblemPage() {
  const problemId = "remove-nth-node-from-end-of-list"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      n = testCase["n"]  # Position from end to remove
      ...
      # Call solution with both head and n parameters
      result = []
      current = solution.${functionName}(head, n)
      while current:
      ...
      if result == testCase['expected']:
          ...
          print(f"Test case {i+1}: Failed | Input: head={testCase['head']}, n={n} | Expected: {testCase['expected']}, Got: {result}")
          return
