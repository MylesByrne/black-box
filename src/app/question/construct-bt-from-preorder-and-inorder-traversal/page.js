...
export default function ProblemPage() {
  const problemId = "construct-binary-tree-from-preorder-and-inorder-traversal"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      preorder = testCase.get("preorder", [])
      inorder = testCase.get("inorder", [])
      
      # Call solution with preorder and inorder arrays
      constructed_root = solution.${functionName}(preorder, inorder)
      
      # Convert the constructed tree to level-order array for comparison
      result = []
      if constructed_root:
          # Level-order traversal
          queue = [constructed_root]
          while queue:
              node = queue.pop(0)
              if node:
      ...
      if result == expected:
          ...
          print(f"Test case {i+1}: Failed | Input: preorder={preorder}, inorder={inorder} | Expected: {expected}, Got: {result}")
          return
