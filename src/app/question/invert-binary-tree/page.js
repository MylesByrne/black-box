...
export default function ProblemPage() {
  const problemId = "invert-binary-tree"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      # Call solution
      inverted_root = solution.${functionName}(root)
      
      # Convert result tree back to array for comparison
      result = []
      if inverted_root:
          # Level-order traversal to convert tree to array
          queue = [inverted_root]
          while queue:
              node = queue.pop(0)
              if node:
      ...
      if result == testCase['expected']:
          ...
          print(f"Test case {i+1}: Failed | Input: root={testCase['root']} | Expected: {testCase['expected']}, Got: {result}")
          return
...
try:
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const testCall = `from collections import deque
from typing import List, Optional
