...
export default function ProblemPage() {
  const problemId = "pacific-atlantic-water-flow"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      heights = testCase.get("heights", [])
      expected = testCase.get("expected", [])
      
      # Call the pacificAtlantic function with the heights matrix
      result = solution.pacificAtlantic(heights)
      
      # Sort both result and expected for consistent comparison
      # Convert lists to tuples for sorting
      sorted_result = sorted(map(tuple, result))
      sorted_expected = sorted(map(tuple, expected))
      
      # Compare sorted results with expected output
