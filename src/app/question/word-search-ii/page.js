...
export default function ProblemPage() {
  const problemId = "word-search-ii"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      words = testCase.get("words", [])
      expected = testCase.get("expected", [])
      
      # Call the findWords function with the board and words
      result = solution.findWords(board, words)
      
      # Sort both lists for consistent comparison (order doesn't matter in this problem)
      sorted_result = sorted(result) if result else []
