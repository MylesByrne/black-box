...
export default function ProblemPage() {
  const problemId = "validate-binary-search-tree"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      # For isValidBST, the result is a boolean
      if str(result).lower() == str(testCase['expected']).lower():
          continue
      else:
          print(f"Test case {i+1}: Failed | Input: root={root_array} | Expected: {testCase['expected']}, Got: {result}")
          return
