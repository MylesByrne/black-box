...
export default function ProblemPage() {
  const problemId = "permutations"; // Hardcoded
...
def check_solution():
  ...
  for i in range(len(testCases)):
      ...
      # Call the permute function with the input array
      result = solution.permute(nums)
      
      # Sort both result and expected for consistent comparison
      # Convert inner lists to tuples for sorting, then sort the list of tuples
      sorted_result = sorted([tuple(perm) for perm in result])
      sorted_expected = sorted([tuple(perm) for perm in expected])
